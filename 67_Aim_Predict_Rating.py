...
data_file = '/Users/grigorev-ee/Work/AnacondaProjects/My_projects/ml_w_python/data/Amazon_Unlocked_Mobile.csv'
...
# appropriate outputs. An MLP consists of multiple layers of nodes in a directed graph, with each layer fully connected to the next one.
# Except for the input nodes, each node is a neuron (or processing element) with a nonlinear activation function. (source Wikipedia)
...
# The forward pass of a RNN is the same as the one of a MLP except that outputs from hidden layers are also used as inputs from the same layer.
# That means that the input from the hidden layer is both the outputs from the hidden layer one step back in time and the external input.
# So we have the equation:

# ah,t=∑iwi,h∗xi,t+∑h′wh′,h∗bh′,t−1

# bh,t=ϕh(ah,t)

# where:

# xt,i=  value of input i at time t
# at,j=  network input to unit j at time t
# bt,j=  output of activation of unit j at time t
# wi,h=  weights of the network
...
#
# Explanation summarized from : https://github.com/fchollet/keras/issues/3110
